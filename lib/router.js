...
Router.map(function() {
  ...
  this.route('/People/:_id/edit', {
    ...
    data: function() {
      ...
      if (admins.includes(this.params._id)) {
        return {
        ...
        // everyone else can only see the projects/sigs they are in
        return {
          person: People.findOne(this.params._id),
          projects: Projects.find({people: this.params._id}),
          sigs: Sigs.find({ people: this.params._id })
        }
      }
    },
    waitOn: function() {
