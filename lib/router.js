...
Router.map(function(){
    ...
    this.route('Sprint/Projects/:proj/latest', {
        template: 'Sprint',
        data: function() {
            var current = Sprints.find({project: this.params.proj, startdate: {$lte: new Date()}, enddate: {$gte: new Date()}});
            if(current.count() == 0){ // return latest
                return Sprints.findOne({project: this.params.proj}, {sort : {startdate:-1}, limit:1});
            }
            return Sprints.findOne({project: this.params.proj, startdate: {$lte: new Date()}, enddate: {$gte: new Date()}});
        },
        waitOn: function() {
    ...
    this.route('/Sprint/edit', {
        name: 'sprintEdit',
        waitOn: function() {
            return [ Meteor.subscribe('sprints'),
                     Meteor.subscribe('globalSprints'),
                     Meteor.subscribe('projects')];
        }
    });
...
Router.onBeforeAction(requireLogin, {only: ['Sprint/Projects/:proj', 'Sprint', 'personEdit', 'rosterEdit', 'applications']});
